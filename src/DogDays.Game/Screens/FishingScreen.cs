...
public sealed class FishingScreen : IGameScreen
{
    ...
    private const int MinimumFishPerSpecies = 1;
    private const int MinimumAdditionalFishCount = 4;
    private const int MaximumAdditionalFishCount = 7;
    private const float PerchPopulationWeight = 0.45f;
    private const float BassPopulationWeight = 0.35f;
    ...
    internal static (int PerchCount, int BassCount, int CatfishCount) RollFishPopulation(Random rng)
    {
        ArgumentNullException.ThrowIfNull(rng);

        var perchCount = MinimumFishPerSpecies;
        var bassCount = MinimumFishPerSpecies;
        var catfishCount = MinimumFishPerSpecies;
        var additionalFishCount = rng.Next(MinimumAdditionalFishCount, MaximumAdditionalFishCount + 1);
        var bassThreshold = PerchPopulationWeight + BassPopulationWeight;

        for (var i = 0; i < additionalFishCount; i++)
        {
            var roll = rng.NextSingle();
            if (roll < PerchPopulationWeight)
            {
                perchCount++;
            }
            else if (roll < bassThreshold)
            {
                bassCount++;
            }
            else
        ...
        return (perchCount, bassCount, catfishCount);
    }

    private void SpawnFish()
    {
        var rng = new Random();
        var population = RollFishPopulation(rng);
        ...
        SpawnSpecies(FishSilhouette.FishType.Perch, population.PerchCount, swimBounds, rng);

        var bassBounds    = swimBounds.SliceHorizontal(0.35f);
        SpawnSpecies(FishSilhouette.FishType.Bass, population.BassCount, bassBounds, rng);

        var catfishBounds = swimBounds.SliceHorizontal(0.55f);
        SpawnSpecies(FishSilhouette.FishType.Catfish, population.CatfishCount, catfishBounds, rng);
    }
    ...
    private void ShowCatchToast(FishSilhouette fish)
    {
        ...
        {
            FishSilhouette.FishType.Perch   => "Perch",
        ...
        {
            FishSilhouette.FishType.Perch   => (0.1f, 0.5f),
