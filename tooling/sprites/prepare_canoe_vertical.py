"""
Prepare the vertical canoe prop sprite from the raw source image.

Steps:
1) Remove edge-connected near-white background pixels.
2) Crop to visible sprite bounds.
3) Resize to two tiles high (64 px) with aspect ratio locked.
...
def main() -> None:
    ...
    final_width = max(1, round(resized.size[0] * (TARGET_HEIGHT / resized.size[1])))
    final = resized.resize((final_width, TARGET_HEIGHT), resample=Image.Resampling.NEAREST)
    final = remove_white_fringe(final)
    final = clean_alpha(final)
    final = trim_to_alpha_bounds(final)

    if final.size[1] != TARGET_HEIGHT:
        corrected_width = max(1, round(final.size[0] * (TARGET_HEIGHT / final.size[1])))
        final = final.resize((corrected_width, TARGET_HEIGHT), resample=Image.Resampling.NEAREST)
