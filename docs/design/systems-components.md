...
| `WatercraftBoardSequence` | Screen-owned canoe ride controller for the player and follower. Consumes a nearby `Watercraft` interaction, runs delayed hop-in/hop-out tweens into directional front/rear canoe seats, paddles the craft from seated input, draws side-view occupants inside the hull, and only returns control to overworld movement when a valid disembark spot is available. |
