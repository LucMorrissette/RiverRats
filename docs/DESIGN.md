...
| **Watercraft props** | TMX `propType` metadata can spawn dedicated `Watercraft` entities for editor-placed surface craft such as canoes; these props can expose facing-aware boarding interactions, seated paddling movement, and per-map restored runtime state | Establishes a focused world-object category for rideable craft without overloading generic decorative props. |
