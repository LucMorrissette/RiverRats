...
public class FishSilhouetteTests
{
    ...
    [Fact]
    public void UpdateAttraction__Twitch__PerchCanStrike__TransitionsToCurious()
    {
        // Perch also has CanStrike=true. Same pattern should work.
        var fish = CreateFish(FishSilhouette.FishType.Perch);
        var lure = LureAt(80);
    ...
    [Fact]
    public void ReelSpeedMultiplier__Perch__IsFastest()
    {
        var perch = CreateFish(FishSilhouette.FishType.Perch);
        var bass = CreateFish(FishSilhouette.FishType.Bass);
        var catfish = CreateFish(FishSilhouette.FishType.Catfish);

        Assert.True(perch.ReelSpeedMultiplier > bass.ReelSpeedMultiplier);
